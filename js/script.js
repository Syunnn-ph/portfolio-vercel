...
function toggleLanguage() {
    const newLang = AppState.currentLang === 'en' ? 'ch' : 'en';
    setLanguage(newLang);
...
function setLanguage(lang) {
    ...
    if (lang === 'ch') {
        html.setAttribute('lang', 'ch');
        body.setAttribute('data-lang', 'ch');
    } else {
...
function updateLanguageUI() {
    const textElements = document.querySelectorAll('[data-text-en], [data-text-ch]');
    textElements.forEach(element => {
        const enText = element.getAttribute('data-text-en');
        const chText = element.getAttribute('data-text-ch');
        if (AppState.currentLang === 'ch' && chText) {
            element.textContent = chText;
        } else if (AppState.currentLang === 'en' && enText) {
            element.textContent = enText;
        }
    });

    const placeholderElements = document.querySelectorAll('[data-placeholder-en], [data-placeholder-ch]');
    placeholderElements.forEach(element => {
        const enPlaceholder = element.getAttribute('data-placeholder-en');
        const chPlaceholder = element.getAttribute('data-placeholder-ch');
        if (AppState.currentLang === 'ch' && chPlaceholder) {
            element.setAttribute('placeholder', chPlaceholder);
    ...
    if (langToggle) {
        ...
        if (langText) {
            langText.textContent = AppState.currentLang === 'en' ? 'CH' : 'EN';
        }
    }
}
...
function handleFormSubmit(e) {
    ...
    const message = AppState.currentLang === 'ch'
        ? '資料成功送出！'
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const barpercent = parseInt(skillItem.getAttribute('data-percent') / skillItem.getAttribute('data-full-percent') * 100);
...
function initProjectAnimations() {
    ...
    document.querySelectorAll('.project-descriptions').forEach(wrapper => {
        const challenges = wrapper.querySelectorAll('.project-description');
        ...
        function goTo(index) {
            ...
            challenges[current].classList.add('deactive');
            current = (index + challenges.length) % challenges.length;
            challenges[current].classList.remove('deactive');
